...
func _ready() -> void:
	...
	filter_scripts.text_changed.connect(_on_filter_scripts_changed)
	
	script_list.clear()
	script_list.item_selected.connect(_on_script_selected)
	
	create_script_button.pressed.connect(_on_create_button_pressed)
	locate_script_button.pressed.connect(_on_locate_button_pressed)

func _update_theme() -> void:
	if (!Engine.editor_hint || !is_inside_tree()):
		return
	
	create_script_button.icon = EditorSettings.get_icon("ScriptCreate", "EditorIcons")
	locate_script_button.icon = EditorSettings.get_icon("Filesystem", "EditorIcons")
...
func _update_script_list() -> void:
	...
	sorted_scripts.sort_custom(_sort_scripts)
...
func _sort_scripts(a : Script, b : Script) -> bool:
	...
	var a_name = a.resource_name if !a.resource_name == "" else a_path.get_file()
	var b_path = b.resource_path
	var b_name = b.resource_name if !b.resource_name == "" else b_path.get_file()
...
func _format_script_text(script : Script, is_changed : bool = false) -> String:
	...
	var script_name = script.resource_name if !script.resource_name == "" else script_path.get_file()
...
func add_script(script : Script) -> void:
	...
	if (script.is_tool()):
		script_list.set_item_icon_modulate(item_index, EditorSettings.get_color("accent_color", "Editor"))
	...
	if (!script.changed.is_connected(_on_script_source_changed)):
		script.changed.connect(_on_script_source_changed.bind(item_index))
